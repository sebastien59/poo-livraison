...
public class Optimizer {
    ...
    public Collection<Point> filtrer(Point origin, Collection<Point> lp, String criteria) {
        ...
        return filtrer(origin, lp, criteria, Constante.SWAP_BODY_CAPACITY, 99999);
    }
    ...
    public Point plusProcheVoisin(Point P) {
        ...
        switch (P.getTypeP()) {
            case "T":
                listePoint = filtrer(P, listePoint, "DCLQH", Constante.SWAP_BODY_CAPACITY * 2 - ((Client)P).getTournee().getQuantiteTotal(), 99999 - ((Client)P).getTournee().getTempsTotal());
                break;
            case "C":
                listePoint = filtrer(P, listePoint, "DTLQH", Constante.SWAP_BODY_CAPACITY - ((Client)P).getTournee().getQuantiteTotal(), 99999 - ((Client)P).getTournee().getTempsTotal());
                break;
            case "SL":
        ...
        if (listePoint.isEmpty()) {
            switch (P.getTypeP()) {
                case "T":
                    listePoint = listeNonTraites;
                    listePoint = filtrer(P, listePoint, "DTCtcH", 0, 99999);
                    if (listePoint.isEmpty()) {/*System.out.println("Returning empty");*/ return new Depot();}
                    break;
                case "C":
